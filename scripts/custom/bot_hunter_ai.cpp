...
struct hunter_bot : public bot_ai
{
    hunter_bot(Creature *c) :bot_ai(c)
    {
        Reset();
        pet = NULL;
    }
   ...
   void CreatePet()
    ...
    void ReceiveEmote(Player *player, uint32 emote)
    {
        ...
        if(emote == TEXTEMOTE_BONK) ((hunter_bot*)m_creature->AI())->listAuras(m_creature);

        //debug to see what auras are on me
        if(emote == TEXTEMOTE_SALUTE) ((hunter_bot*)m_creature->AI())->listAuras(player);
...
CreatureAI *GetAI_hunter_bot(Creature *pCreature)
{
    return new hunter_bot(pCreature);
}

void AddSC_hunter_bot()
{
    outstring_log("hunterBot added");
    Script *newscript;
    newscript = new Script;
    newscript->Name = "hunter_bot";
    newscript->GetAI = GetAI_hunter_bot;
