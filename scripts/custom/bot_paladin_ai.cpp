...
struct paladin_bot : public bot_ai
{
    paladin_bot(Creature *c) :bot_ai(c)
    {
        Reset();
    }
    ...
    bool CureTarget(Unit *target)
    {
        ...
        {
//error_log ("PaladingBotAI.CureTarget: curing %s of withering heat");
            doCast(target, CLEANSE);
        }
        if (HasAuraName(target, "Ancient Dread"))
        {
//error_log ("PaladingBotAI.CureTarget: curing %s of Ancient Dread");
            doCast(target, CLEANSE);
        }
                if (HasAuraName(target, "Ancient Dread"))
        {
//error_log ("PaladingBotAI.CureTarget: curing %s of Ancient Dread");
        ...
        {
//error_log ("PaladingBotAI.CureTarget: curing %s of Arcane Buffet");
            doCast(target, CLEANSE);
        }
        if (HasAuraName(target, "Shadow Buffet"))
        {
//error_log ("PaladingBotAI.CureTarget: curing %s of Shadow Buffet");
            doCast(target, CLEANSE);
        }
        if (HasAuraName(target, "Flame Buffet"))
        {
//error_log ("PaladingBotAI.CureTarget: curing %s of Flame Buffet");
            doCast(target, CLEANSE);
        }
        if (HasAuraName(target, "Frost Buffet"))
        {
//error_log ("PaladingBotAI.CureTarget: curing %s of Frost Buffet");
            doCast(target, CLEANSE);
        }
        return true;
    }
...
CreatureAI *GetAI_paladin_bot(Creature *pCreature)
{
    return new paladin_bot(pCreature);
}

void AddSC_paladin_bot()
{
    outstring_log("paladinBot added");
    Script *newscript;
    newscript = new Script;
    newscript->Name = "paladin_bot";
    newscript->GetAI = GetAI_paladin_bot;
