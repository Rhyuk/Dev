...
struct rogue_bot : public bot_ai
{
    rogue_bot(Creature *c) : bot_ai(c) 
    {
        Reset();
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (isTimerReady(Eviscerate_Timer) && comboPoints>0) {
            ...
            float minDmg = m_creature->GetWeaponDamageRange(BASE_ATTACK, MINDAMAGE);
            int damage = irand(int32(base_attPower*5*0.03f),int32(base_attPower*5*0.07f))+minDmg+m_creature->getLevel();
//error_log ("EVISCERTE: base_attPower = %u", base_attPower);
//error_log("\tminDmg = %f", minDmg);
//error_log("\tmaxDmg = %f", m_creature->GetWeaponDamageRange(BASE_ATTACK, MAXDAMAGE));
//error_log ("\tMINDAMAGE after setting it = %f", m_creature->GetWeaponDamageRange(BASE_ATTACK, MINDAMAGE));

// compensate for lack of attack power
//error_log ("\tdamage before = %u", damage);
            damage = damage*(rand()%4+1);
//error_log ("\tdamage after = %u", damage);
...
CreatureAI *GetAI_rogue_bot(Creature *pCreature)
{
    return new rogue_bot(pCreature);
}

void AddSC_rogue_bot()
{
    outstring_log("rogueBot added");
    Script *newscript;
    newscript = new Script;
    newscript->Name = "rogue_bot";
    newscript->GetAI = GetAI_rogue_bot;
