...
bool bot_ai::listAuras (Unit *unit)
{
    ...
    for(Unit::AuraMap::const_iterator itr = vAuras.begin(); itr!=vAuras.end(); ++itr)
    {
        ...
        const std::string name = spellInfo->SpellName[loc];
        outstring_log("aura = %u %s", spellInfo->Id, name.c_str());
    }
    return false;
}
...
//error_log ("val2 = %f", val2);
//error_log ("str = %f", m_creature->GetStat(STAT_STRENGTH));
    //CreatureInfo const *cinfo = objmgr.GetCreatureTemplate(m_creature->GetEntry());
    //m_creature->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, cinfo->maxdmg/mylevel);

    //Custom weapon system
    val2 = abs(val2 + (val2 * (level/80)));
   // error_log ("custom val2 = %f", val2);
    ...
    //error_log ("udpating BASE ATTACK");
    //error_log ("mLevelMult = %f", mLevelMult);
    //error_log ("bot_ai - base_attPower = %f", base_attPower);
    //error_log ("actpower multiplier = %f", attPowerMultiplier);
    //error_log ("actpower val2 = %f", val2);
    //error_log ("\tmin damage = %f", uint32(val2 * attPowerMultiplier)+level);
    //error_log ("\tmax damange = %f",uint32(val2 * attPowerMultiplier)*2+level);
    //error_log ("\tmax damange 2 = %f",uint32(val2 * attPowerMultiplier)*mLevelMult);
...
//error_log ("\t%s - min damage after setting it = %f", m_creature->GetName(), m_creature->GetFloatValue(UNIT_FIELD_MINDAMAGE));
//error_log ("\t%s - MINDAMAGE after setting it = %f", m_creature->GetName(), m_creature->GetWeaponDamageRange(BASE_ATTACK, MINDAMAGE));
