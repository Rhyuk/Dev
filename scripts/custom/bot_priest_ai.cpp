...
struct priest_bot : public bot_ai
{
    priest_bot(Creature *c) :bot_ai(c)
    {
        Reset();
    }
...
//buff and heal master's group
    ...
    {
        ((priest_bot*)m_creature->AI())->doCast(player, SPELL_FORTITUDE, true);
        ((priest_bot*)m_creature->AI())->doCast(player, SPELL_PRAYER_OF_SHADOW, false);
    }

}; //end priest_bot


CreatureAI *GetAI_priest_bot(Creature *pCreature)
{
    return new priest_bot(pCreature);
}


void AddSC_priest_bot()
{
    outstring_log("priestBot added");
    Script *newscript;
    newscript = new Script;
    newscript->Name = "priest_bot";
    newscript->GetAI = GetAI_priest_bot;
