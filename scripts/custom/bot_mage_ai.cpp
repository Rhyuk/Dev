...
struct mage_bot : public bot_ai
{
    mage_bot(Creature *c) :bot_ai(c)
    {
        //master = NULL;
        Reset();
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if(!m_creature->isInCombat())
        {
            ...
            {
                //error_log("mana is %u, total mana is %u", m_creature->GetPower(POWER_MANA), m_creature->GetMaxPower(POWER_MANA));
    ...
    void DoNormalAttack(const uint32 diff)
    {
        ...
        //error_log("%s: state = %u, next_state = %u, next_state_time = %u", m_creature->GetName(), state, next_state, next_state_timer);
    ...
    {
        ((mage_bot*)m_creature->AI())->doCast(player, ARCANEINTELLECT, true );
    }
};

CreatureAI *GetAI_mage_bot(Creature *pCreature)
{
    return new mage_bot(pCreature);
}

void AddSC_mage_bot()
{
    outstring_log("mageBot added by Brats reworked by Azrael with the help of Machiavelli");
    Script *newscript;
    newscript = new Script;
    newscript->Name = "mage_bot";
    newscript->GetAI = GetAI_mage_bot;
