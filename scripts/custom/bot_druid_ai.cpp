...
struct bot_druid_ai : public bot_ai
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        for(Group::member_citerator itr = a.begin(); itr != a.end(); itr++)
        {
            ...
            {
                //error_log("Druid_Bot: healing someoneelse: %s it has %d HP", tPlayer->GetName(), tPlayer->GetHealth());
...
CreatureAI *GetAI_druid_bot(Creature *pCreature)
{
...
{
    outstring_log("druidBot added");
    Script *newscript;
    newscript = new Script;
    newscript->Name = "druid_bot";
    newscript->GetAI = GetAI_druid_bot;
