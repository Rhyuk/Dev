...
struct boss_ignis_AI : public BossAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
		...
		if (add_timer <= diff)
		{
			Creature * add = me->SummonCreature(id_add, me->GetPositionX(), me->GetPositionY() , me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000);
			add->setFaction(16);
			add->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
