...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options)
{
    ...
    if(GetGroup() && HaveBot())
    {
        //error_log("Player::teleporting.. removing from group");
...
void Player::GetBotLevelInfo(uint32 race, uint32 class_,uint32 level, PlayerLevelInfo* info) const {
    objmgr.GetPlayerLevelInfo (race, class_, level, info);
}
    ...
    if (isInFlight())
    {
        if (HaveBot())
        {
            if (GetGroup())
            {
                ...
                if (m_group->IsMember(m_bot->GetGUID()))
                {
                    if (m_group->RemoveMember(m_bot->GetGUID(), 99) <= 1)
                    {
...
//error_log ("putting priest to follow master");
  //          m_bot->GetMotionMaster()->MoveFollow(this, urand(5, 10), PET_FOLLOW_ANGLE);
    ...
    if(!isInFlight())
    {
        ...
        {
            m_bot->Unmount();
            CreatureInfo const *cinfo = objmgr.GetCreatureTemplate(m_bot->GetEntry());
        ...
        switch(GetBotClass())
        {
            case CLASS_DRUID: //DRUID FORM SUPPORT
            {
                ...
                {
                    //flight form
                    if((((IsMounted()) && (m_bot->getLevel() >= 70)) || (m_form == FORM_FLIGHT) || (m_form == FORM_FLIGHT_EPIC)))
                    {
...
*/
                ...
                {
                    //combat form is not handled in AI
                }
                else if((GetBotMustWaitForSpell1() == 0) && (((IsMounted()) && (m_bot->getLevel() < 60)) || (m_form == FORM_TRAVEL)) && (!m_bot->isInFlight()))
                {
                    //travel form
                ...
                else if((GetBotMustWaitForSpell1() == 0) && (m_form == FORM_CAT) && (!m_bot->isInFlight()))
                {
                    //cat form
                ...
                else if((GetBotMustWaitForSpell1() == 0) && (m_form == FORM_BEAR) && (!m_bot->isInFlight()))
                {
                    //bear form
                ...
                //RESET FORMS
                if((!m_bot->isInFlight()) && (!m_bot->IsInWater())  &&  (!m_bot->isInCombat())  && (!IsMounted()) && (m_form != FORM_TRAVEL) && (m_form != FORM_CAT))
                {
...
void Player::CreateBot(uint32 botentry, uint8 botrace, uint8 botclass)
{
    ...
    //error_log("Player::CreateBot: %s", newbot->GetName());
    //error_log("\thp = %d", newbot->GetMaxHealth());
    //error_log("\tmana = %d", newbot->GetMaxPower(POWER_MANA));
    //error_log("\tlevel = %d", newbot->getLevel());
    //error_log("\tentry = %d", botentry);
    //error_log("\trace = %d", botrace);
    //error_log("\tclass = %d", botclass);
   ...
   // m_bot->SelectLevel(objmgr.GetCreatureTemplate(m_bot->GetEntry()));
   // m_bot->SelectLevel(objmgr.GetCreatureTemplate(m_bot->GetEntry()));
    CreatureBaseStats const* stats = objmgr.GetCreatureBaseStats(getLevel(), newbot->GetCreatureInfo()->unit_class);
...
std::list<std::string> *Player::GetCharacterList()
{
    ...
    do {
        ...
        names->insert(names->end(), fields[0].GetCppString());
...
void Player::RemoveFromGroup(Group* group, uint64 guid)
{
    if (group)
    {
        ...
        if(_player!=NULL) {
            ...
            if (!GetPlayer(guid)->GetGroup() || group->GetMembersCount()==0) return;
        }
...
bool Player::RewardHonor(Unit *uVictim, uint32 groupsize, float honor, bool pvptoken)
{
    ...
    if (honor <= 0)
    {
        ...
        if (uVictim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if(pVictim->IsPlayerbot() && (!sWorld.getConfig(CONFIG_HONOR_FROM_PLAYERBOTS) ||
