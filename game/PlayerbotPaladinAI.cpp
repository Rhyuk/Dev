...
Limitations:    - Talent build decision is made by key talent spells, which makes them viable only after level 50-ish.. Until then default behaviour is Retribution type
                - Tanking bots should taunt if any group member is under attack, currently only saves master
                - Should check high threat generators and cast HOS if needed.
                - Retadin's support roles are not fully covered.. For example -> HOP casting
                - Situations needing CLEANSE and HOF casting : limited / non-existant..
                - Holy shock is not a hostile or friendly spell: fails CanCast checks and can not be casted either way..
...
void PlayerbotPaladinAI::DoNextCombatManeuver(Unit *pTarget)
{
    ...
    //uint32 ccSpells[6] = { CLEANSE, HOF, R_ESCAPE_ARTIST, R_EVERY_MAN_FOR_HIMSELF, R_WILL_OF_FORSAKEN, R_STONEFORM };
    ...
    // Paladin always enters melee combat
    ...
    switch(m_role)
    {
        ...
        case BOT_ROLE_OFFTANK:
            ...
            if (CastSpell(HOLY_SHIELD,m_bot)) { return; }
        ...
        case BOT_ROLE_DPS_MELEE:
            ...
            // Dps specials
            if (pThreat < threatThreshold && !m_bot->HasAura(FORBEARANCE) && CastSpell(AVENGING_WRATH,m_bot)) {} //no gcd
            break;
        #pragma endregion
    }
    ...
    else if (m_role != BOT_ROLE_SUPPORT && ai->GetManaPercent() > 30 && DoSupportRaid(m_bot,30,false,false,false)) { return; }
...
bool PlayerbotPaladinAI::CureTarget(Unit *target)
{
   ...
   // if (target->HasAuraType( SPELL_AURA_PERIODIC_DAMAGE))  if(HasAuraName(target, "Infected Bite") || HasAuraName(target, "Ancient Dread") || HasAuraName(target, "Infected Wound"))  return CastSpell(CLEANSE, target);
    return false;
...
#pragma region Choose Buff > Class
    switch(target->getClass())
    {
        case CLASS_MAGE:
        case CLASS_WARLOCK:
            if (CanCast(BOW,target) && !HasAuraName(target,BOW) && !HasAuraName(target,GBOW) ) return CastSpell(BOW,target,false);
            else if (CanCast(BOK,target) && !HasAuraName(target,BOK) && !HasAuraName(target,GBOK) ) return CastSpell(BOK,target,false);
            else if (CanCast(BOS,target) && !HasAuraName(target,BOS) && !HasAuraName(target,GBOS) ) return CastSpell(BOS,target,false);
            //return(!HasAuraName(target, "Blessing of Wisdom") && CastSpell (BLESSING_OF_WISDOM, target));
            break;        
        case CLASS_PRIEST:
            if (CanCast(BOK,target) && !HasAuraName(target,BOK) && !HasAuraName(target,GBOK) ) return CastSpell(BOK,target,false);
            else if (CanCast(BOW,target) && !HasAuraName(target,BOW) && !HasAuraName(target,GBOW) ) return CastSpell(BOW,target,false);
            else if (CanCast(BOS,target) && !HasAuraName(target,BOS) && !HasAuraName(target,GBOS) ) return CastSpell(BOS,target,false);
            break;
        case CLASS_HUNTER:
            if (CanCast(BOM,target) && !HasAuraName(target,BOM) && !HasAuraName(target,GBOM) ) return CastSpell(BOM,target,false);
            else if (CanCast(BOK,target) && !HasAuraName(target,BOK) && !HasAuraName(target,GBOK) ) return CastSpell(BOK,target,false);
            else if (CanCast(BOW,target) && !HasAuraName(target,BOW) && !HasAuraName(target,GBOW) ) return CastSpell(BOW,target,false);
            else if (CanCast(BOS,target) && !HasAuraName(target,BOS) && !HasAuraName(target,GBOS) ) return CastSpell(BOS,target,false);
            break;
        case CLASS_ROGUE:
            ...
            //else if (CanCast(BOW,target) && !HasAuraName(target,BOW) && !HasAuraName(target,GBOW) ) return CastSpell(BOW,target);
            else if (CanCast(BOS,target) && !HasAuraName(target,BOS) && !HasAuraName(target,GBOS) ) return CastSpell(BOS,target,false);
            break;
        case CLASS_WARRIOR:
        case CLASS_DEATH_KNIGHT:
            ...
            {
                if (CanCast(BOS,target) && !HasAuraName(target,BOS) && !HasAuraName(target,GBOS) ) return CastSpell(BOS,target,false);
                else if (CanCast(BOK,target) && !HasAuraName(target,BOK) && !HasAuraName(target,GBOK) ) return CastSpell(BOK,target,false);
                else if (CanCast(BOM,target) && !HasAuraName(target,BOM) && !HasAuraName(target,GBOM) ) return CastSpell(BOM,target,false);
                //else if (CanCast(BOW,target) && !HasAuraName(target,BOW) && !HasAuraName(target,GBOW) ) return CastSpell(BOW,target);
            }
            else
            {
        ...
        case CLASS_PALADIN:
            ...
            break;
            //return(!HasAuraName(target, "Blessing of Sanctuary") && CastSpell (BLESSING_OF_SANCTUARY, target));
...
bool PlayerbotPaladinAI::ChangeAura(uint32 aura)
{
    ...
    if(m_bot->HasAura(aura)) 
    {
        ...
        //else if (aura == RETRIBUTION_AURA) return ChangeAura(DEVOTION_AURA);
        //else if (aura == CONCENTRATION_AURA) return ChangeAura(DEVOTION_AURA);
        //else if (aura == CRUSADER_AURA) return ChangeAura(DEVOTION_AURA);
