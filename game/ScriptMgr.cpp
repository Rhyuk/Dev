...
bool ScriptMgr::GossipHelloScriptId (Player * pPlayer, Creature* pCreature, uint32 scriptId)
{
...
bool ScriptMgr::GossipSelect(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    ...
    Script *tmpscript = m_scripts[pCreature->GetScriptId()];
    if (uiSender > 6000) tmpscript = m_scripts[objmgr.GetScriptId("script_bot_giver")];
