...
uint32 PlayerbotAI::getSpellIdExact(const char *args, bool includePassive, bool master)
{
    ...
    //outstring_log("PBot Class %u - Found in Search - [%u/%s]", m_bot->getClass(), foundSpellId, namepart.c_str());
    if (foundSpellId > 70000) { outstring_log("CRITICAL: PBot Class %u - Weird Spell in Search - [%u/%s]", m_bot->getClass(), foundSpellId, namepart.c_str()); }
    return foundSpellId;
}
        ...
        case CMSG_MOVE_SPLINE_DONE:
        {
            ...
            for(PlayerBotMap::const_iterator it = masterSession.GetPlayerBotsBegin(); it != masterSession.GetPlayerBotsEnd(); ++it)
            {
                ...
                if(!bot->IsInMap((WorldObject*) masterSession.GetPlayer()))
                {
                    ...
                    uint32 mountDisplayId = objmgr.GetTaxiMountDisplayId(sourcenode, bot->GetTeam());
        ...
        case CMSG_GAMEOBJ_USE:
            {
               ...
               for(PlayerBotMap::const_iterator it = masterSession.GetPlayerBotsBegin(); it != masterSession.GetPlayerBotsEnd(); ++it) 
                {
                    Player* const bot = it->second;
//error_log ("gameobject type = %u", obj->GetGoType());
...
void PlayerbotAI::HandleBotOutgoingPacket(const WorldPacket &packet)
{
    switch(packet.GetOpcode())
    {
         ...
         {
             m_bot->GetSession()->SendTradeStatus(4);
             return;
         }
        ...
        {
            //outstring_log("Bot [%u] SMSG_SPELL_DELAYED",m_bot->GetGUIDLow());
            WorldPacket p(packet);
            ...
            //outstring_log("Bot [%u] caster [%u] Spell Delayed [%u]",m_bot->GetGUIDLow(), casterGuid, delayTime);
        ...
        case SMSG_FORCE_RUN_SPEED_CHANGE:
        {
            ...
            //uint64 guid; p >> guid;
            Player *tPlayer = objmgr.GetPlayer(guid);
            if(!tPlayer) return;
        ...
        case SMSG_TRADE_STATUS:
        {
            ...
            if(status == 4) { // TRADE_STATUS_TRADE_ACCEPT
                if (!m_bot->isTradeAccepted() || !m_bot->GetTrader()->isTradeAccepted()) {
        ...
        case SMSG_TEXT_EMOTE:
        {
            ...
            switch(text_emote)
            {
                case TEXTEMOTE_BOW:
                {
                    ...
                    Player *const bot =objmgr.GetPlayer(pPlayer->GetSelection());
        ...
        case SMSG_MESSAGECHAT:
        {
            ...
            Player * fromPlayer = objmgr.GetPlayer(guid);
...
void PlayerbotAI::UseMount() const
{
    ...
    {
// error_log ("PlayerbotAI::UseMount: %s is mounted but %s is not", m_master->GetName(), m_bot->GetName());
        //Player Part
...
//error_log ("master_speed1 = %d", master_speed1);
//error_log ("master_speed2 = %d", master_speed2);
        //Bot Part
        uint32 spellMount = 0;
        for(PlayerSpellMap::iterator itr = m_bot->GetSpellMap().begin(); itr != m_bot->GetSpellMap().end(); ++itr)
        {
...
//error_log ("spellMount = %u", spellMount);
...
//else error_log ("%s is attacking %s", m_bot->GetName(), thingToAttack->GetName());
        }
    }
    ...
    //default to master's current victim
    if(!thingToAttack || thingToAttack->GetVisibility() == VISIBILITY_OFF) thingToAttack = m_master->getVictim();
...
//error_log ("current target doesn't make sense so following");
        Follow(*m_master);
        return;
    }
...
void PlayerbotAI::UpdateAI(const uint32 p_time)
{
    ...
    if (m_playerBotsFly==0 && m_master->isInFlight())
    {
        ...
        {
            const WorldLocation fakeloc = WorldLocation(35, -0.873190, 52.920242, -27.550674, 1.655620);
    ...
    else if(m_bot->isInCombat()) GetCombatOrders(); //if bot is in combat but master is not, attack

    else if(m_bot->getStandState() == UNIT_STAND_STATE_SIT) Feast(); //are we sitting, if so feast if possible
    ...
    {
//error_log ("%s: %s is too far away so following", m_bot->GetName(), m_master->GetName());
        Follow(*m_master);
    }
}
...
error_log ("I dont have spell");
    ...
    if (!pSpellInfo)
    {
        outstring_log("CRITICAL: PBot Class %u - Non-existing Spell in Cast - [%u]", m_bot->getClass(), spellId);
        std::stringstream ss;
...
bool PlayerbotAI::CastSpell(const SpellEntry * pSpellInfo, Unit *target, bool checkFirst, bool castExistingAura, bool skipFriendlyCheck, bool skipEquipStanceCheck, bool triggered)
{
    ...
    if(!pSpellInfo)
    {
        error_log ("%s: Missing spell entry in CastSpell Direct", m_bot->GetName());
    ...
    {
        //NEGATIVE SPELL
        if (spellmgr.GetSpellCustomAttr(spellId) & SPELL_ATTR_CU_NEGATIVE)
        {
    ...
    //outstring_log("Bot [%u] Start Spell [%u] Cast Time [%f]", m_bot->GetGUIDLow(), pSpellInfo->Id, psCastTime);
...
bool PlayerbotAI::CanCast(const SpellEntry * pSpellInfo, Unit *target, bool castExistingAura, bool skipFriendlyCheck, bool skipEquipStanceCheck)
{
    if (!pSpellInfo)
    {
        outstring_log("CRITICAL: PBot Class %u - Non-existing Spell in CastCheck - Direct SpellEntry", m_bot->getClass());
        TellMaster("Missing spell entry in CastSpell");
        SetIgnoreUpdateTime(1);
        return false;
    }
    ...
    {
        uint32 formMask = (GetForm() ? 1 << (GetPlayerBot()->m_form - 1) : 0);
        //outstring_log("DEBUG: Spell [%u] - Form [%X] - Need Form [%X] - Not Form [%X]", pSpellInfo->Id, formMask, pSpellInfo->Stances, pSpellInfo->StancesNot );
        if (pSpellInfo->Stances & formMask) { return true; }
        if (pSpellInfo->StancesNot && pSpellInfo->StancesNot & formMask) { return false; }
        if (!m_bot->HasItemFitToSpellReqirements(pSpellInfo)) return false;
    }
...
void PlayerbotAI::HandleCommand(const std::string &text, Player &fromPlayer)
{
    ...
    else if (text == "Thank you for this treat")
    {
        ...
        if (trade) {
            ...
            WorldPacket *p = new WorldPacket(CMSG_INITIATE_TRADE, 8);
            //m_bot->GetSession()->HandleAcceptTradeOpcode(*p);  // p not used
        ...
        text == "quests")
    {
        ...
        for (QuestStatusMap::iterator iter=m_bot->getQuestStatusMap().begin(); iter!=m_bot->getQuestStatusMap().end(); ++iter)
        {
            ...
            else if (qData->m_status == QUEST_STATUS_COMPLETE && !qData->m_rewarded)
            {
                out << "Quest " << " |cffffffff|Hquest:" << questId << "|h[" << name << "]|h|r" << " ";
                out << " complete" << "\n";
            }
...
//        SetQuestNeedItems();
    ...
    if(!m_lootCurrent)
    {
        ...
        if(!m_lootCurrent)
        {
            //error_log("PlayerbotAI::DoLoot() error location #1, please report this error immediately!");
            return false;
        }
        ...
        if(m_bot->IsWithinDistInMap(object, INTERACTION_DISTANCE))
        {
            ...
            uint32 lootNum = loot->GetMaxSlotInLootFor(m_bot);
//error_log("[PlayerbotAI]: %s loot target 0x%08X got %d items", m_bot->GetName(), m_lootCurrent, loot->GetMaxSlotInLootFor(m_bot));
            ...
            for(uint32 l = 0; l < lootNum; ++l)
            {
                ...
                if(m_needItemList[item->itemid] > 0)
                {
//error_log("[PlayerbotAI]: %s LOOT needed item 0x%04X", m_bot->GetName(), item->itemid);
                    ItemPosCountVec dest;
                    if(m_bot->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, item->itemid, item->count) == EQUIP_ERR_OK)
                    {
                        Item *newitem = m_bot->StoreNewItem(dest, item->itemid,true, item->randomPropertyId);
