...
bool PlayerbotClassAI::TakePosition(Unit *followTarget, BotRole bRole, float bDist, float bMinDist, float bMaxDist, float bAngle, Unit *faceTarget)
{
    ...
    if (doFollow)
    {
            ...
            )
        {
            ...
            //error_log("Bot[%u] is moving, curDist[%f], bDist[%f], bminDist[%f], bMaxDist[%f], bAngle[%f], InFront[%u]", m_bot->GetGUIDLow(), curDist, bDist,bMinDist, bMaxDist, bAngle, followTarget->HasInArc(M_PI,m_bot));
...
uint8 PlayerbotClassAI::GetHealthPercentRaid(Player *gPlayer, uint8 &countNeedHealing)
{
    ...
    if(!unitList.empty()){
      for (std::list<Unit*>::iterator itr = unitList.begin() ; itr!=unitList.end();++itr) {
        ...
        Unit *tPlayer = objmgr.GetPlayer((*itr)->GetGUID());
        ...
        //outstring_log("me = %s, checked= %s %u [%u / %u]", myname.c_str(), hisname.c_str(), fndHPPercent, tPlayer->GetHealth(), tPlayer->GetMaxHealth());
...
Unit *PlayerbotClassAI::FindMainTankInRaid(Player *gPlayer)
{
    ...
    {
       QueryResult_AutoPtr result = CharacterDatabase.PQuery("SELECT memberGuid FROM group_member WHERE memberFlags='%u' AND guid = '%u'",MEMBER_FLAG_MAINTANK, pGroup->GetGUID());
        if(result)
        {    
            uint64 pGuid = MAKE_NEW_GUID(result->Fetch()->GetInt32(),0,HIGHGUID_PLAYER);
            pPlayer = objmgr.GetPlayer(pGuid);
    ...
    if (!unitList.empty()){
      for (std::list<Unit*>::iterator itr = unitList.begin() ; itr!=unitList.end();++itr) {
        ...
        {
            Pet* tpet = ObjectAccessor::GetPet(tPlayer->GetPetGUID());
...
Unit *PlayerbotClassAI::FindMainAssistInRaid(Player *gPlayer)
{
    ...
    if (pGroup->isRaidGroup())
    {
          QueryResult_AutoPtr result = CharacterDatabase.PQuery("SELECT memberGuid FROM group_member WHERE memberFlags='%u' AND guid = '%u'",MEMBER_FLAG_MAINASSIST, pGroup->GetGUID());
          if(result)
        {    
            uint64 pGuid = MAKE_NEW_GUID(result->Fetch()->GetInt32(),0,HIGHGUID_PLAYER);
            pPlayer = objmgr.GetPlayer(pGuid);
...
Player * PlayerbotClassAI::FindMage(Player *gPlayer)
{
    ...
    {
        Player *tPlayer = objmgr.GetPlayer(itr->guid);
