...
/*
Name : PlayerbotShamanAI.cpp
Complete: maybe around 70%

Limitations:    - Talent build decision is made by key talent spells, which makes them viable only after level 50-ish..Until then default behaviour is Enhancement type.
                - Totems are casted one by one, and totem summoning algorithm is too much resource heavy..
                - Shaman Weapon temp enchants are not really casted, instead applied directly to weapon.
                - Rockbiter weapon is too problematic with missing enchant spell ids etc. So not included..
                - Rockbiter weapon, Stoneclaw totem, resist totems, Totem group calls, water walking and breathing are not used right now..
...
void PlayerbotShamanAI::DoNextCombatManeuver(Unit *pTarget)
{
    ...
    if (m_tank->GetGUID() == m_bot->GetGUID()) { m_role=BOT_ROLE_TANK; } // Hey! I am Main Tank
        ...
        if (OwnPartyHP < 40 && ai->GetManaPercent() >= 30) {m_role = BOT_ROLE_SUPPORT;}
    ...
    if (m_bot->HasAura(ELEMENTAL_MASTERY) || m_bot->HasAura(NATURES_SWIFTNESS) ) 
    {
        if ((isUnderAttack(m_tank,3) || m_tank->GetGUID() == m_bot->GetGUID()) && CastSpell(CHAIN_LIGHTNING,pTarget)) { return; }
        if (CanCast(LAVA_BURST,pTarget,true) && pTarget->HasAura(FLAME_SHOCK,m_bot->GetGUID()) && CastSpell(LAVA_BURST,pTarget,false)) { return; }
        if (CastSpell(LIGHTNING_BOLT,pTarget,true,true)) { return; }
    }
    ...
    if (isUnderAttack(m_tank,5) && CastSpell(FERAL_SPIRIT,m_bot)) { return; }
...
void PlayerbotShamanAI::DoNonCombatActions()
{
    ...
    //Collect totems
    // if (HasAuraName(m_bot,"Strength of Earth") && CanCast(TOTEMIC_RECALL, m_bot, true, true) && CastSpell(TOTEMIC_RECALL,m_bot)) { return; }
...
/*bool PlayerbotShamanAI::FindMount()
{
    if(GHOST_WOLF)
    {
        if(GetPlayerBot()) CastSpell(GHOST_WOLF, GetPlayerBot());
        return true;
    } else return false;
}

bool PlayerbotShamanAI::Unmount()
{
    GetPlayerBot()->RemoveAurasDueToSpell(GHOST_WOLF);
    return true;
}

bool PlayerbotShamanAI::IsMounted()
{
    return GetPlayerBot()->IsMounted() || HasAuraName(GetPlayerBot(), GHOST_WOLF);
}*/
...
bool PlayerbotShamanAI::ChangeTotems(uint32 mode)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (!tEntry) continue;
        //uint32 totemsSpell = totemEntry->spells[0];
        //if (!totemsSpell) continue;
        //const std::string tsname = tSpell->SpellName[0];
        //if (sizeof (tsname) < 8) continue;
        //uint16 tsnamelen = tsname.length();
        //Get rid of " Totem" part of the spell..
...
bool PlayerbotShamanAI::ChangeWeaponEnchants()
{
    ...
    {    
        ...
        if (!isUnderAttack()) { mhEnch = EARTHLIVING_WEAPON; }
    }
    else 
    { 
        if (WINDFURY_WEAPON) 
        ...
        //else if (ROCKBITER_WEAPON) { if (CanCast(ROCKBITER_WEAPON,m_bot) && CastSpell(ROCKBITER_WEAPON,m_bot)) { return; } } //rockbiter is too complicated to cast
    }
