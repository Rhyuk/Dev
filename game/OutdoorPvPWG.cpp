...
void OutdoorPvPWG::HandlePlayerEnterZone(Player * plr, uint32 zone)
{
    ...
    if (isWarTime())
    {
		...
		if(plr->HasAuraType(SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED) ||plr->HasAuraType(SPELL_AURA_MOD_INCREASE_FLIGHT_SPEED) || plr->HasAuraType( SPELL_AURA_MOD_MOUNTED_FLIGHT_SPEED_ALWAYS))
	{		
		plr->Whisper("Alerte : Vous survolez une zone en cours de bataille, vous allez être téléporté !",LANG_YOU , plr->GetGUID());
		plr->AddAura(58730,plr);
...
void OutdoorPvPWG::UpdateClock()
{
    ...
    if (!isWarTime())
	 ...
	 if ((m_timer>1800000) && (m_timer<1802000) && (m_wartime==false)) 
       sWorld.SendZoneText(ZONE_DALARAN, objmgr.GetTrinityStringForDBCLocale(LANG_BG_WG_DALARAN_ANNOUNCE));
